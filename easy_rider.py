...
def check_arrival_times(bus_data: list[Bus]) -> None:

    for bus in bus_data:

        # Get stops for this bus sorted by ID
    stops = sorted(bus_data, key=lambda x: x.stop_id if x.bus_id == bus.bus_id else float('inf'))  

    prev_time = stops[0].a_time

    for i in range(1, len(stops)):
        current = stops[i]

        if current.bus_id != bus.bus_id:
            continue

        if current.a_time <= prev_time:
            print(f"Arrival time test:\nbus_id line {bus.bus_id}: wrong time on station {current.stop_name}")
        break

        prev_time = current.a_time
